...
#ifdef HH_VSYNC_ISSUE
    /*
     * This is a workaround for b/25845510. If we don't have resync samples
     * after many presents, something is wrong with HW vsync.  Tell SF to
     * disable HW vsync now and re-enable it next time.
     */
    if (!mNumResyncSamples && mNumPresentWithoutResyncSamples++ >
         MAX_PRESENT_WITHOUT_RESYNC_SAMPLES) {
        mNumPresentWithoutResyncSamples = 0;
        return false;
    }
#endif
